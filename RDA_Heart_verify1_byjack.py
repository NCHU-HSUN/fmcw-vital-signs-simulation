...
R_fs: float = 25e+6;                       # 取樣率(sampling rate), 25 MHz
...
num_loops: float = 1;                       # frameCfg 中的 1 loops
...
frame_Fs: float = 1/frame_periodicity;      # 12.5 Hz
frame_len: int = 128;
...
fig_range = plt.figure(num=1, figsize=(10, 6));
ax = fig_range.add_subplot(111, projection='3d');
...
fig_phase = plt.figure(num=2, figsize=(10, 4))
...
# FFT 前套用 Hann window，降低有限長資料造成的頻譜洩漏。
spectrum_window = np.hanning(frame_len)
f1 = np.fft.rfftfreq(frame_len, d=frame_periodicity)
fft_resp = np.abs(np.fft.rfft(resp_filtered * spectrum_window))
fft_heart = np.abs(np.fft.rfft(heart_filtered * spectrum_window))

resp_band = (f1 >= 0.1) & (f1 <= 0.5)
...
plt.plot(f1, fft_resp, 'b-')
plt.xlabel('Frequency (Hz)')
plt.ylabel('Magnitude')
plt.xlim(0, frame_Fs / 2)
# plt.ylim(0, 4)
plt.title('Respiration Spectrum')
plt.grid(True) # 加上網格方便觀察峰值

# --- 繪製圖 3：心跳頻譜 ---
fig_heart = plt.figure(num=4, figsize=(8, 4))
plt.plot(f1, fft_heart, 'r-')
plt.xlabel('Frequency (Hz)')
plt.ylabel('Magnitude')
plt.xlim(0, frame_Fs / 2)
# plt.ylim(0, 4)
plt.title('Heartbeat Spectrum')
plt.grid(True)


# 將所有圖表存入固定資料夾；tight_layout 可避免座標標籤被裁切。
figures = {
    "01_range_profile.png": fig_range,
    "02_phase_vibration.png": fig_phase,
    "03_respiration_spectrum.png": fig_resp,
    "04_heartbeat_spectrum.png": fig_heart,
}
for filename, figure in figures.items():
    figure.tight_layout()
    figure.savefig(OUTPUT_DIR / filename, dpi=180, bbox_inches="tight")

print(f"已將 {len(figures)} 張圖片儲存至：{OUTPUT_DIR}")
plt.close("all")
